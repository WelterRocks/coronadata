...
class MQTT
{
    ...
    function __construct($config = ".coronadatarc", $hostname = null, $port = null, $username = null, $password = null, $client_id = null, $topic = null)
    {
...
//        $this->mqtt->onConnect(array($this, 'subscribe'));        
    ...
    public function get_topic($prefix, $continent, $country, $suffix)
    {
        $topic = $this->config->mqtt_topic;

        $topic = str_replace("%prefix%", $prefix, $topic);
        $topic = str_replace("%continent%", $continent, $topic);
        $topic = str_replace("%country%", $country, $topic);
